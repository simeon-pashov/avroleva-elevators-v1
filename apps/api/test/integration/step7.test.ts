...
describe('settings: bank details and billing defaults', () => {
  it('rejects a bad IBAN checksum with a field error and keeps the rest', async () => {
    ...
    expect(res.body.fields.map((f: { path: string }) => f.path)).toContain('settings.billing.bank.iban')
  ...
  it('GET /billing/config exposes states, transitions, system stages and providers as data', async () => {
    ...
    expect(res.body.stages.map((s: { key: string; position: number }) => `${s.position}:${s.key}`)).toEqual([
      '1:reminder',
      '2:second_reminder',
      '3:final_notice',
    ])
    ...
    const providers = Object.fromEntries(res.body.providers.map((p: { name: string; enabled: boolean }) => [p.name, p.enabled]))
...
describe('scheduled billing run', () => {
  it('runs on the run day for the current month, is idempotent, and waits before the run day', async () => {
    ...
    await prismaBase.tenant.update({
      where: { id: A.tenantId },
      data: { settings: { billing: { runDay: 20, dueDays: 10, bank: { beneficiary: 'Тест Лифт ЕООД', iban: 'BG80BNBG96611020345678', bic: 'BNBGBGSD', bankName: 'БНБ' } } } },
    })
    ...
    await prismaBase.tenant.update({
      where: { id: A.tenantId },
      data: { settings: { billing: { runDay: 1, dueDays: 10, bank: { beneficiary: 'Тест Лифт ЕООД', iban: 'BG80BNBG96611020345678', bic: 'BNBGBGSD', bankName: 'БНБ' } } } },
    })
    ...
    const issued = await prismaBase.domainEvent.count({ where: { tenantId: A.tenantId, type: 'InvoiceIssued' } })
  ...
  it('quarterly and exempt contract overrides', async () => {
    ...
    const r1 = await request(server).post('/api/v1/billing/invoices/generate').set(bearer(A.ownerToken)).send({ period: '2026-04' })
    expect(r1.status).toBe(201)
    const quarterly = r1.body.invoices.find((i: { contractId: string }) => i.contractId === q.body.id)
    expect(quarterly).toBeDefined()
    expect(quarterly.periodEnd).toBe('2026-06-30')
    expect(quarterly.amountCents).toBe(9000)
    expect(r1.body.invoices.find((i: { contractId: string }) => i.contractId === ex.body.id)).toBeUndefined()
    const r2 = await request(server).post('/api/v1/billing/invoices/generate').set(bearer(A.ownerToken)).send({ period: '2026-05' })
    expect(r2.body.invoices.find((i: { contractId: string }) => i.contractId === q.body.id)).toBeUndefined()
    await request(server).patch(`/api/v1/contracts/${q.body.id}`).set(bearer(A.ownerToken)).send({ billing: null }).expect(200)
    await request(server).patch(`/api/v1/contracts/${ex.body.id}`).set(bearer(A.ownerToken)).send({ status: 'terminated', endDate: '2026-01-31' })
  })
})
...
describe('dunning as data', () => {
  ...
  it('the daily job moves an overdue invoice through the stages, notifies, and stops when paid', async () => {
    ...
    const r = await request(server).post('/api/v1/billing/invoices/generate').set(bearer(A.ownerToken)).send({ period: '2026-01' })
    expect(r.status).toBe(201)
    const jan = r.body.invoices.find((i: { contractId: string }) => i.contractId === contractId)
    overdueId = jan.id
    await prismaBase.invoice.update({ where: { id: overdueId }, data: { dueAt: new Date(addDays(today, -20) + 'T00:00:00Z') } })
    const preview = await request(server).get('/api/v1/billing/dunning/preview').set(bearer(A.ownerToken))
    expect(preview.status).toBe(200)
    const item = preview.body.items.find((i: { invoiceId: string }) => i.invoiceId === overdueId)
    expect(item).toMatchObject({ currentStage: 0, nextStageKey: 'second_reminder', nextStagePosition: 2, channel: 'email' })
    ...
    const email = await waitFor(() =>
      prismaBase.notification.findFirst({ where: { tenantId: A.tenantId, eventType: 'DunningStageReached', channel: 'email', relatedId: overdueId } }),
    )
    ...
    const inApp = await waitFor(() =>
      prismaBase.notification.findFirst({ where: { tenantId: A.tenantId, eventType: 'DunningStageReached', channel: 'in_app', relatedId: overdueId } }),
    )
    expect(inApp.link).toBe(`/invoices/${overdueId}`)
    const event = await prismaBase.domainEvent.findFirst({ where: { tenantId: A.tenantId, type: 'DunningStageReached', aggregateId: overdueId } })
    expect(event!.payload).toMatchObject({ stageKey: 'second_reminder', templateKey: 'dunning_second', channel: 'email' })
    const audit = await prismaBase.auditLog.findFirst({ where: { tenantId: A.tenantId, action: 'invoice.dunning' } })
  ...
  it('a tenant-customised schedule with an enabled late fee adds an adjustment, never edits the invoice', async () => {
    const rule = await request(server)
      ...
      .set(bearer(A.ownerToken))
      .send({ kind: 'flat', amountCents: 1500, percentBp: 0, graceDays: 25, capCents: 2000, enabled: true })
    ...
    const stages = await request(server)
      ...
      .send({
        stages: [
          { key: 'reminder', offsetDays: 3, channel: 'email', templateKey: 'dunning_reminder', lateFeeRuleKey: null, active: true },
          { key: 'stern', offsetDays: 19, channel: 'in_app', templateKey: 'dunning_final', lateFeeRuleKey: 'late_fee', active: true },
        ],
      })
    expect(stages.status, stages.text).toBe(200)
    expect(stages.body.stages.map((s: { key: string; position: number }) => `${s.position}:${s.key}`)).toEqual(['1:reminder', '2:stern'])
    ...
    await prismaBase.invoice.update({ where: { id: overdueId }, data: { dunningStage: 1, dueAt: new Date(addDays(today, -40) + 'T00:00:00Z') } })
    ...
    const adj = await prismaBase.invoiceAdjustment.findMany({ where: { tenantId: A.tenantId, invoiceId: overdueId } })
    ...
    const detail = await request(server).get(`/api/v1/billing/invoices/${overdueId}`).set(bearer(A.ownerToken))
    ...
    const contactEmails = await prismaBase.notification.count({ where: { tenantId: A.tenantId, eventType: 'DunningStageReached', channel: 'email', relatedId: overdueId } })
    expect(contactEmails).toBe(1)
    // Restore the system schedule for the rest of the suite.
    await request(server).put('/api/v1/billing/dunning-stages').set(bearer(A.ownerToken)).send({ stages: [] }).expect(200)
    expect((await request(server).get('/api/v1/billing/config').set(bearer(A.ownerToken))).body.stagesCustomised).toBe(false)
    await request(server).put('/api/v1/billing/late-fee-rules/late_fee').set(bearer(A.ownerToken)).send({ enabled: false }).expect(200)
  })
  it('paying the invoice stops dunning; a credit note reduces the balance and emits CreditNoteIssued', async () => {
    ...
    const after = await request(server).get(`/api/v1/billing/invoices/${overdueId}`).set(bearer(A.ownerToken))
    ...
    await request(server)
      ...
      .expect(400)
    const ev = await prismaBase.domainEvent.findFirst({ where: { tenantId: A.tenantId, type: 'CreditNoteIssued' } })
    ...
    const preview = await request(server).get('/api/v1/billing/dunning/preview').set(bearer(A.ownerToken))
    expect(preview.body.items.find((i: { invoiceId: string }) => i.invoiceId === overdueId)).toBeUndefined()
    expect((await billing.runDunning(A.tenantId)).reached).toBe(0)
    // Second credit note number follows gaplessly.
    const inv2 = (await prismaBase.invoice.findFirst({ where: { tenantId: A.tenantId, status: { in: ['issued', 'overdue', 'partially_paid'] } } }))!
...
describe('payments, references and partial / over-payments', () => {
  it('partial payment -> partially_paid; the rest -> paid; over-payment leaves an unallocated remainder', async () => {
    const inv = (await prismaBase.invoice.findFirst({ where: { tenantId: A.tenantId, status: 'issued' }, orderBy: { number: 'asc' } }))!
    ...
    const payments = await prismaBase.payment.findMany({ where: { tenantId: A.tenantId, paidAt: new Date(today + 'T00:00:00Z') }, orderBy: { createdAt: 'asc' } })
    const remainder = payments.find((p) => p.invoiceId === null && p.amountCents === 700)
    expect(remainder).toBeDefined()
    expect(remainder!.note).toContain('overpayment')
    const detail = await request(server).get(`/api/v1/billing/invoices/${inv.id}`).set(bearer(A.ownerToken))
    expect(detail.body.payments.map((p: { amountCents: number }) => p.amountCents).sort()).toEqual([1000, open - 1000].sort())
  ...
  it('the list filters by q (number / reference) and by dunning stage; bulk pay', async () => {
    const list = await request(server).get('/api/v1/billing/invoices').query({ q: 'AE-' }).set(bearer(A.ownerToken))
    expect(list.status).toBe(200)
    expect(list.body.items.length).toBeGreaterThan(2)
    const byNumber = await request(server).get('/api/v1/billing/invoices').query({ q: '1' }).set(bearer(A.ownerToken))
    expect(byNumber.body.items.some((i: { number: number }) => i.number === 1)).toBe(true)
    const staged = await request(server).get('/api/v1/billing/invoices').query({ dunningStage: 1 }).set(bearer(A.ownerToken))
    expect(staged.body.items.every((i: { dunningStage: number }) => i.dunningStage >= 1)).toBe(true)
    const pending = await request(server).get('/api/v1/billing/invoices').query({ pending: true }).set(bearer(A.ownerToken))
    const ids = pending.body.items.map((i: { id: string }) => i.id)
    expect(ids.length).toBeGreaterThan(0)
    const bulk = await request(server).post('/api/v1/billing/invoices/bulk').set(bearer(A.ownerToken)).send({ action: 'pay', ids })
    expect(bulk.status, bulk.text).toBe(200)
    expect(bulk.body.done).toBe(ids.length)
    expect((await request(server).get('/api/v1/billing/invoices').query({ pending: true }).set(bearer(A.ownerToken))).body.items).toHaveLength(0)
  })
})
...
describe('bank-statement import -> match -> statement balance', () => {
  ...
  it('preview auto-matches by reference and by amount + name, keeps the rest unallocated', async () => {
    ...
    const gen = await request(server).post('/api/v1/billing/invoices/generate').set(bearer(A.ownerToken)).send({ period: '2025-11' })
    expect(gen.status).toBe(201)
    refInvoice = gen.body.invoices.find((i: { contractId: string }) => i.contractId === contractId)
    const gen2 = await request(server).post('/api/v1/billing/invoices/generate').set(bearer(A.ownerToken)).send({ period: '2025-12' })
    nameInvoice = gen2.body.invoices.find((i: { contractId: string }) => i.contractId === contractId)
    const text = [
    ...
    expect(res.body.rows[0]).toMatchObject({ matchKind: 'reference', status: 'proposed', invoiceId: refInvoice.id, invoiceNumber: refInvoice.number })
    expect(res.body.rows[1]).toMatchObject({ matchKind: 'amount_name', status: 'proposed', invoiceId: nameInvoice.id })
    ...
    const presets = await request(server).get('/api/v1/billing/bank-imports/presets').set(bearer(A.ownerToken))
    expect(presets.body.items).toHaveLength(3)
  })
  it('manual match, commit, statement running balance, PaymentMatched events', async () => {
    const imp = await request(server).get(`/api/v1/billing/bank-imports/${importId}`).set(bearer(A.ownerToken))
    ...
    expect(matched.body.rows[2]).toMatchObject({ status: 'matched', matchKind: 'manual', buildingId })
    const commit = await request(server).post(`/api/v1/billing/bank-imports/${importId}/commit`).set(bearer(A.ownerToken))
    expect(commit.status, commit.text).toBe(200)
    expect(commit.body).toEqual({ importId, booked: 3, unallocated: 0, ignored: 0 })
    await request(server).post(`/api/v1/billing/bank-imports/${importId}/commit`).set(bearer(A.ownerToken)).expect(409)
    const ref = await request(server).get(`/api/v1/billing/invoices/${refInvoice.id}`).set(bearer(A.ownerToken))
    expect(ref.body.status).toBe('paid')
    expect(ref.body.payments[0]).toMatchObject({ source: 'bank_import', reference: refInvoice.paymentReference, counterparty: 'Някой друг' })
    const matchedEvents = await prismaBase.domainEvent.count({ where: { tenantId: A.tenantId, type: 'PaymentMatched' } })
    expect(matchedEvents).toBe(3)
    const officeRow = await waitFor(() =>
      prismaBase.notification.findFirst({ where: { tenantId: A.tenantId, eventType: 'PaymentMatched', channel: 'in_app' } }),
    )
    expect(officeRow.subject).toContain('Плащане')

    const st = await request(server).get(`/api/v1/buildings/${buildingId}/statement`).query({ from: '2025-01-01', to: today }).set(bearer(A.ownerToken))
    expect(st.status, st.text).toBe(200)
    expect(st.body.openingBalanceCents).toBe(0)
    const debits = st.body.lines.filter((l: { kind: string }) => l.kind === 'invoice' || l.kind === 'late_fee').reduce((s: number, l: { debitCents: number }) => s + l.debitCents, 0)
    const credits = st.body.lines.filter((l: { kind: string }) => l.kind === 'payment' || l.kind === 'credit_note').reduce((s: number, l: { creditCents: number }) => s + l.creditCents, 0)
    ...
    const print = await request(server).get(`/print/statement/${buildingId}`).query({ from: '2025-01-01', to: today }).set(bearer(A.ownerToken))
  ...
  it('a later manual match on a committed import books the payment immediately', async () => {
    const text = 'Date,Amount,Currency,Counterparty,Details,Reference\n06/09/2026,5.00,EUR,"Anon","nothing",Z1\n'
    const res = await request(server).post('/api/v1/billing/bank-imports/preview').set(bearer(A.ownerToken)).send({ filename: 'x.csv', text, preset: 'en_comma_signed' })
    expect(res.status, res.text).toBe(201)
    const id = res.body.id
    const commit = await request(server).post(`/api/v1/billing/bank-imports/${id}/commit`).set(bearer(A.ownerToken))
    expect(commit.body).toEqual({ importId: id, booked: 0, unallocated: 1, ignored: 0 })
    const rowId = res.body.rows[0].id
    const later = await request(server).put(`/api/v1/billing/bank-imports/${id}/rows/${rowId}`).set(bearer(A.ownerToken)).send({ buildingId })
    expect(later.body.rows[0]).toMatchObject({ status: 'booked' })
    expect(later.body.rows[0].paymentId).toBeTruthy()
    await request(server).put(`/api/v1/billing/bank-imports/${id}/rows/${rowId}`).set(bearer(A.ownerToken)).send({ ignore: true }).expect(409)
  })
  it('unreadable files answer 400', async () => {
    await request(server).post('/api/v1/billing/bank-imports/preview').set(bearer(A.ownerToken)).send({ filename: 'x.csv', text: 'a,b\n1,2\n', mapping: undefined }).expect(400)
  })
})
...
describe('invoice document, EPC QR and the public page', () => {
  ...
  it('detail and print carry the bank block, the reference and an EPC SVG while open', async () => {
    const gen = await request(server).post('/api/v1/billing/invoices/generate').set(bearer(A.ownerToken)).send({ period: '2025-10' })
    openId = gen.body.invoices.find((i: { contractId: string }) => i.contractId === contractId).id
    const detail = await request(server).get(`/api/v1/billing/invoices/${openId}`).set(bearer(A.ownerToken))
  ...
  it('the public QR page shows the payment block only with an open balance and the setting on', async () => {
    await request(server).patch('/api/v1/tenant').set(bearer(A.ownerToken)).send({ features: { publicQrPage: true } }).expect(200)
    ...
    await request(server)
      .patch('/api/v1/tenant')
      .set(bearer(A.ownerToken))
      .send({ settings: { billing: { ...me.body.settings.billing, showPaymentOnPublicPage: false } } })
      .expect(200)
    ...
    await request(server)
      .patch('/api/v1/tenant')
      .set(bearer(A.ownerToken))
      .send({ settings: { billing: { ...me.body.settings.billing, showPaymentOnPublicPage: true } } })
      .expect(200)
  })
  it('statement e-mail: sent with the attachment, logged as a statement run', async () => {
    ...
    expect(res.status, res.text).toBe(201)
    expect(res.body).toMatchObject({ kind: 'statement', status: 'sent', sentTo: 'petya@example.com' })
    ...
    await request(server).post(`/api/v1/reports/statement/${bBuildingId}/send`).set(bearer(A.ownerToken)).send({}).expect(404)
  })
})
...
describe('demo mode and the demo payment page', () => {
  ...
  it('the demo provider is refused without demoMode; with it a link is created and the page records a payment', async () => {
    ...
    await request(server)
      ...
      .expect(200)
    openId = (await prismaBase.invoice.findFirst({ where: { tenantId: A.tenantId, status: { in: ['issued', 'overdue', 'partially_paid'] } } }))!.id
    const refused = await request(server).post(`/api/v1/billing/invoices/${openId}/payment-link`).set(bearer(A.ownerToken))
    expect(refused.status).toBe(409)
    expect(refused.body.code).toBe('billing.provider.demoModeOff')
    // Owners cannot switch demoMode on; the platform admin can.
    const adminSet = await request(server).post(`/api/v1/admin/tenants/${A.tenantId}/demo-mode`).set('Authorization', `Bearer ${admin}`).send({ enabled: true })
    expect(adminSet.status, adminSet.text).toBe(200)
    expect(adminSet.body.features.demoMode).toBe(true)
    const link = await request(server).post(`/api/v1/billing/invoices/${openId}/payment-link`).set(bearer(A.ownerToken))
    ...
    const paid = await request(server).post(path).type('form').send({ cardName: 'Test Card', cardNumber: '4242 4242 4242 4242' })
    expect(paid.status, paid.text).toBe(200)
    const inv = await request(server).get(`/api/v1/billing/invoices/${openId}`).set(bearer(A.ownerToken))
    ...
    expect((await prismaBase.payment.count({ where: { tenantId: A.tenantId, provider: 'demo' } }))).toBe(1)
    // Demo mode off again: the page is a 404 even for a known token.
    await request(server).post(`/api/v1/admin/tenants/${A.tenantId}/demo-mode`).set('Authorization', `Bearer ${admin}`).send({ enabled: false }).expect(200)
  ...
  it('webhooks for the stub providers answer 501 / 404', async () => {
    await request(server).post('/webhooks/payments/stripe').set('Content-Type', 'application/json').send('{}').expect(501)
  ...
  it('the admin generates a year of demo data for an empty tenant; reset requires demoMode', async () => {
    ...
    const r = await request(server).post(`/api/v1/admin/tenants/${C.tenantId}/demo-data`).set('Authorization', `Bearer ${admin}`).send({ months: 4 })
    ...
    const pending = await prismaBase.eventDelivery.count({
      where: { status: 'pending', eventId: { in: (await prismaBase.domainEvent.findMany({ where: { tenantId: C.tenantId }, select: { id: true } })).map((e) => e.id) } },
    })
    expect(pending).toBe(0)
    const invoices = await request(server).get('/api/v1/billing/invoices').query({ limit: 200 }).set(bearer(C.ownerToken))
    ...
    expect(invoices.body.items.every((i: { paymentReference: string }) => /^AE-\d{4}-\d{6}$/.test(i.paymentReference))).toBe(true)
    // Second run is a no-op.
    const again = await request(server).post(`/api/v1/admin/tenants/${C.tenantId}/demo-data`).set('Authorization', `Bearer ${admin}`).send({})
    expect(again.body.skipped).toBe(true)
    await request(server).post(`/api/v1/admin/tenants/${C.tenantId}/demo-data`).set('Authorization', `Bearer ${admin}`).send({ reset: true }).expect(409)
    ...
    await request(server).post(`/api/v1/admin/tenants/${C.tenantId}/demo-mode`).set('Authorization', `Bearer ${admin}`).send({ enabled: true }).expect(200)
    const firstInvoice = await prismaBase.invoice.findFirst({ where: { tenantId: C.tenantId } })
    const job = (await runJob('tenancy.demoReset')) as Record<string, { files?: number; error?: string }>
    ...
    expect(await prismaBase.auditLog.count({ where: { tenantId: C.tenantId, action: 'tenant.demoReset' } })).toBe(1)
  ...
  it('generateDemoData on a tenant with its own registry adds operational data only', async () => {
    ...
    await request(server).post('/api/v1/contracts').set(bearer(D.ownerToken)).send({ customerId: cust.id, buildingId: b.id, startDate: '2025-06-01', lines: [{ elevatorId: e.id, monthlyPriceCents: 4000 }] }).expect(201)
...
describe('tenant isolation and roles for every new endpoint', () => {
  it('B answers 404 on A resources; a technician gets 403', async () => {
    ...
    await request(server).post(`/api/v1/billing/invoices/${inv.id}/credit-notes`).set(b).send({ reason: 'xx' }).expect(404)
    ...
    await request(server).post(`/api/v1/billing/invoices/${inv.id}/pay`).set(b).send({ paidAt: today }).expect(404)
    await request(server).get(`/api/v1/billing/bank-imports/${imp.id}`).set(b).expect(404)
    await request(server).put(`/api/v1/billing/bank-imports/${imp.id}/rows/${row.id}`).set(b).send({}).expect(404)
    ...
    await request(server).post(`/api/v1/reports/statement/${buildingId}/send`).set(b).send({}).expect(404)
    const bulk = await request(server).post('/api/v1/billing/invoices/bulk').set(b).send({ action: 'pay', ids: [inv.id] })
    expect(bulk.body).toMatchObject({ done: 0, skipped: 1 })
    expect((await request(server).get('/api/v1/billing/bank-imports').set(b)).body.items).toHaveLength(0)
    ...
    const tech = await request(server)
      .post('/api/v1/users')
      .set(bearer(A.ownerToken))
      .send({ username: `tech7_${Date.now() % 100000}`, password: 'password123', name: 'Монтьор', role: 'technician' })
    expect(tech.status, tech.text).toBe(201)
    const login = await request(server).post('/api/v1/auth/login').set(CSRF).send({ username: tech.body.username, password: 'password123' })
    ...
    await request(server).put('/api/v1/billing/dunning-stages').set(t).send({ stages: [] }).expect(403)
