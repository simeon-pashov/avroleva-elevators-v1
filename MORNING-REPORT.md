...
| 7 Billing that runs itself (2026-09-09) | Scheduled billing run and dunning as data (stages, late fee), credit notes, bank-statement import with auto-matching, statement per building (print + e-mail), EPC QR + IBAN on every document and the public page, payment provider port (demo adapter, IRIS/Stripe stubs), demo mode with a one-command year of data and nightly reset, invoices list + detail + bank import + billing settings UI. Product renamed to Avroleva Elevators. | `HANDOFF-STEP7.md`, `docs/adr/0001-billing-jobs-payments.md`, `billing-settings.png`, `invoice-qr.png`, `bank-import.png`, `statement.png` |
