...
Страницата `https://srv1662742.hstgr.cloud/avroleva/elevators-v1/downloads/` показва същите стъпки и QR код
(офисът я отваря от „Потребители“ → „Изтегли приложението за Android“).

1. Отворете на телефона `https://srv1662742.hstgr.cloud/avroleva/elevators-v1/downloads/` (или сканирайте QR
...
**Release APK** (2026-09-10 04:45): `D:\Code\Avroleva\Avroleva Elevators\Releases\avroleva-elevators-tech-0.6.0.apk`,
...
- Inside the APK: 25 web files under `assets/public/`; the two `index-*.js` chunks contain
  `https://srv1662742.hstgr.cloud/avroleva/elevators-v1` and `"0.6.0"`; `index.html` registers no service worker.
...
- Install, launch: splash → Enroll screen, "Сървър" prefilled with the baked-in
  `https://srv1662742.hstgr.cloud/avroleva/elevators-v1`, v0.6.0.
...
- App Links (`https://…/avroleva/elevators-v1/tech/?enroll=`) open the app automatically on Android 12+ only
  after `/.well-known/assetlinks.json` is published at the **host root** (not under `/avroleva/elevators-v1/`,
...
3. Verify `https://srv1662742.hstgr.cloud/avroleva/elevators-v1/downloads/` renders with the size/date line and
...
4. Real-phone pass: install from the page (note every OS prompt for the Bulgarian instructions),
   enroll by scanning the office QR (server must prefill to `https://srv1662742.hstgr.cloud/avroleva/elevators-v1`),
