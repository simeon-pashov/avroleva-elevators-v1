...
| jobs | job, job_line, job_event, job_stage* | create, lines, markQuoted, sendQuote, transition(stage, evidence), schedule, start, addNote, complete (repair visit through the VisitRecorder port), invoice (InvoiceIssuer port -> billing.issueInvoice), reject, cancel, revise · get, list/board, per elevator, byOrigin, summary (open quotes, awaiting approval, scheduled this week, done-not-invoiced), listForSync, config (stages as data) — HANDOFF-STEP8 | JobCreated, JobQuoted, JobApproved, JobScheduled, JobStarted, JobCompleted, JobInvoiced, JobRejected, JobCancelled, JobApprovalReminder | — (ports to visits, billing, notifications wired in `wiring.ts`) |
...
|---|---|---|---|
| A1 | **Ports & adapters** for every external thing: `SmsSender`, `EmailSender`, `ViberSender`, `InvoicingProvider`, `PdfRenderer`, `FileStorage`, `Geocoder` (geocode + as-you-type `search`), `MapTiles`, `Clock`. One interface each in `platform/ports/`, adapters in `platform/adapters/<port>/<vendor>.ts`, chosen by env (`SMS_PROVIDER=console`, `twilio` or `smsapi`); every port has a `console`/`memory` adapter for tests and demos. | platform | "Viber is replaced by WhatsApp" → new `WhatsAppSender` adapter, rule table points at it; "inv.bg instead of our PDFs" → `InvoicingProvider=invbg`; "disk full" → `FileStorage=s3`. |
...
**Capacitor (step 10, built):** the same `apps/tech` code ships inside Capacitor 7 (`apps/tech/android`, appId `bg.avroleva.elevators.tech`) from a separate build (`npm run build:native`: `base: '/'`, `VITE_NATIVE=1`, no service worker, `dist-native/`); the PWA build at `/tech/` is untouched. The server address is runtime configuration — build-time default `VITE_DEFAULT_API_ORIGIN`, overridable on the Enroll screen / in Settings, persisted in `secureStorage`, prefilled from the office QR (`…/tech/?enroll=` → everything before `/tech/`) or the custom scheme `avroleva-elevators://enroll?server&token`. The WebView origin is `https://localhost` (androidScheme https, cleartext off); the API allows exactly `https://localhost`, `capacitor://localhost`, `http://localhost` with credentials (`platform/http/cors.ts`) and the app always sends `Authorization: Bearer` (no cookies). Platform seams in `apps/tech/src/platform/`: `camera`, `geolocation`, `secureStorage`, `http`, `share` plus `network` and `appHost` (resume, deep links, back button, splash) — web implementations are the default, native ones (`platform/native/*`, mapping 1:1 to `@capacitor/camera|geolocation|preferences|share|network|app`, dynamically imported) are chosen when `Capacitor.isNativePlatform()`. The native camera hands a `File` to the same downscale + sha256 pipeline. Distribution: signed APK sideloaded from `/downloads/` (`DATA_DIR/releases/tech.apk`), see `docs/ANDROID-RELEASE.md`; store accounts later. Rejected: React Native/Flutter (a second codebase); loading the remote URL inside the wrapper (no offline, store-rejection risk).
...
- **nginx path prefix:** `location /avroleva/elevators-v1/ { proxy_pass http://127.0.0.1:3005/; ... client_max_body_size 25M; proxy_read_timeout 120s; }` inside the existing `listen 443 ssl` server block; `nginx -t && systemctl reload nginx`; verify the other five apps still return 200. The app serves `/` (office SPA), `/m/` (technician PWA, own `manifest.webmanifest` and SW scope), `/q/:code` (public page), `/api/v1/`. SPAs built with `VITE_BASE=/avroleva/elevators-v1/` and `/avroleva/elevators-v1/m/`; server links from `BASE_PATH=/avroleva/elevators-v1` and `PUBLIC_BASE_URL=https://srv1662742.hstgr.cloud` (origin only - the app appends `BASE_PATH`).
- **Custom domain — buy it before the first technician installs and before the first QR labels are printed.** A PWA's identity and its IndexedDB are bound to the origin; moving from `srv…/avroleva/elevators-v1/m/` to `app.avroleva.bg` later means every phone reinstalls and re-syncs, and printed QR codes must be redirected forever. The path prefix is right for internal demos; the founding customer starts on the domain (new `server {}` block, certbot for the new hostname — allowed by the guide — `VITE_BASE=/`, same container).
